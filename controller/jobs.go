...
func (r *JobRepo) Add(job *ct.Job) error {
	...
	case ct.JobStateDown:
		if job.ExitStatus != nil && *job.ExitStatus > 0 || job.HostError != nil {
...
func (r *JobRepo) ListPendingRequests() ([]*ct.ExpandedJobRequest, error) {
	rows, err := r.db.Query("job_request_list_pending")
	if err != nil {
		return nil, err
	}
...
func (c *controllerAPI) ListPendingJobRequests(ctx context.Context, w http.ResponseWriter, req *http.Request) {
	list, err := c.jobRepo.ListPendingRequests()
	if err != nil {
		respondWithError(w, err)
		return
	}
